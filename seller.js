...
function displayProducts(filteredProducts) {
    ...
    // filter to get only on seller products and only approved products from admin
    let filtered = products.filter(p => p.sellerId === 2 && (!p.current_status || p.current_status === "approved"));
...
function updateProduct(btn) {
    ...
    // add binding status
    products[index].current_status = "binding";
...
// for binding and rejected products container
function displayBindingAndRejected() {
    let products = JSON.parse(localStorage.getItem("products")) || [];
    let bindingContainer = document.getElementById("bindingContainer");
    let rejectedContainer = document.getElementById("rejectedContainer");
    ...
    products.forEach(product => {
        ...
        } else if (product.current_status === "rejected") {
            rejectedContainer.innerHTML += `
