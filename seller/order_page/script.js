window.addEventListener("DOMContentLoaded", function () {
  function runOrdersPage() {
    ...
    const sellerOrders = orders
      .map((order) => {
        const filteredItems = order.order_details.items.filter(
    ...
    function renderOrders() {
      let searchValue =
        document.getElementById("order_search")?.value.toLowerCase() || "";
      let statusValue =
        document.querySelector(".dropdown-wrapper select")?.value || "all";
      let productValue =
        document.getElementById("product_search")?.value.toLowerCase().trim() ||
        "";
      ...
      sellerOrders.forEach((order) => {
        let filteredItems = order.items.filter((item) => {
          ...
          const matchOrderId = `${order.id}-${item.productId}`
            .toLowerCase()
          ...
          const matchProduct =
            productValue === "" ||
            item.name.toLowerCase().includes(productValue);
    ...
    // Filter Status
    document
      .querySelector(".dropdown-wrapper select")
    ...
    if (update && cancelBtn && saveBtn && statusSpan && statusSelect) {
      document.querySelectorAll(".order_icons i").forEach((icon) => {
        icon.addEventListener("click", (e) => {
          ...
          let orderId =
            currentOrderEl.querySelector(".order_id h4").textContent;
          let currentStatus = currentOrderEl
            .querySelector(".order_id p.status")
            .textContent.trim();
